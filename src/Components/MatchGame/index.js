...
class MatchGame extends Component {
  ...
  componentWillUnMount = () => {
    const {isMatchRunning} = this.state
  ...
  clickingImages = id => {
    const {displayedPicture} = this.state
    if (displayedPicture.id === id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        displayedPicture: imagesList[Math.ceil(Math.random() * 30)],
      }))
    } else {
      clearInterval(this.timerId)
      this.setState({isMatchRunning: false})
  ...
  playagain = () => {
    this.setState({
      isMatchRunning: true,
      selectedTab: tabsList[0].tabId,
      displayedPicture: imagesList[Math.ceil(Math.random() * 29)],
      score: 0,
      timeLeft: 60,
    })
    this.timerId = setInterval(this.tick, 1000)
  ...
  render() {
    ...
    const filteredList = imagesList.filter(
      eachlist => eachlist.category === selectedTab,
    )
    return (
